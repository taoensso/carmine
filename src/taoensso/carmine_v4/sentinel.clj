...
(deftest ^:private _addr-utils
  [(let [sm (add-addrs->back nil [["ip1" 1] ["ip2" "2"] ^{:server-name "server3"} ["ip3" 3]])
         sm (add-addr->front sm  ["ip2" 2])
         sm (add-addrs->back sm [["ip3" 3] ["ip6" 6]])]

     [(is (= sm [["ip2" 2] ["ip1" 1] ["ip3" 3] ["ip6" 6]]))
      (is (= (mapv opts/descr-sock-addr sm)
            [["ip2" 2] ["ip1" 1] ["ip3" 3 {:server-name "server3"}] ["ip6" 6]]))])

   (let [sm (add-addrs->back nil [["ip4" 4] ["ip5" "5"]])
         sm (remove-addr     sm   ["ip4" 4])]
     [(is (= sm [["ip5" 5]]))])])
...
(deftest ^:private _unique-addrs
  [(is (= (unique-addrs {:m1 {:master [1 1] :sentinels [[1 1] [1 2] [2 2]]}
...
(deftest ^:private _parse-nodes-info->addrs
  [(is (= (parse-nodes-info->addrs
...
(deftype SentinelSpec
  ...
  Object
  (toString [this] ; "SentinelSpec[masters=3, replicas=2, sentinels=6]"
              ...
              (let [t0-attempt (System/currentTimeMillis)
                ...
                (if-let [[reporting-sentinel-addr resolved-addr confirmed-role]
                  ...
                  (let [{:keys [resolve-timeout-ms retry-delay-ms]} sentinel-opts
                        ...
                        retry-at-ms (+ elapsed-ms (or retry-delay-ms 0))]

                    (if (> retry-at-ms (or resolve-timeout-ms 0))
                      (do
                        (vswap! attempt-log_ conj
                          [:timeout
                           (str
...
(defn ^:public sentinel-spec
  ...
  See also `load-edn-config` for a util to load `sentinel-addrs-map`
  from a system property / Env var / resource file."
